...
// This function not used if a firebase token is registered
      ...
      if (/ios/i.test(device.platform)) {
        ...
        this.push.setApplicationIconBadgeNumber(() => {
          ...
          this.emit('error', {
            ...
            type: 'cordova.' + device.platform.toLowerCase(),
    ...
    if (!this.configured) {
      ...
      Meteor.startup(() => {
        ...
        if (typeof PushNotification !== 'undefined') {
          ...
          // cordovaOptions object used to init phonegap-plugin-push
          var cordovaOptions = options.cordovaOptions || {};
          this.push = PushNotification.init(cordovaOptions);
          ...
          this.push.on('registration', (data) => {
            ...
            if (data && data.registrationId && this.token !== data.registrationId) {
              ...
              var token;
              if (data.registrationType && data.registrationType.toLowerCase() === 'fcm') {
                // If cordova-plugin-push reports this is an fcm token
                token = { fcm: data.registrationId };
              } else {
                // Else use apn for iOS / gcm for Android
                token = { [getService()]: data.registrationId };
              }
